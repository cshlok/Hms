...
const MedicationGivenSchema = z.record(z.string(), z.any()).optional().nullable();

export const ProgressNoteSchema = z.object({
  admission_id: z.number().int().positive(),
  note_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Date must be in YYYY-MM-DD format"), // Assuming YYYY-MM-DD format
  note_time: z.string().regex(/^\d{2}:\d{2}(:\d{2})?$/, "Time must be in HH:MM or HH:MM:SS format"), // Assuming HH:MM or HH:MM:SS format
