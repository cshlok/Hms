...
async function getAppointmentsFromDB(filters: any) {
  ...
  return mockAppointments.filter(appointment => {
    ...
    // Apply doctor filter
    if (filters.doctorId && appointment.doctor_id.toString() !== filters.doctorId) return false;
    
    // Apply patient filter
    if (filters.patientId && appointment.patient_id.toString() !== filters.patientId) return false;
...
async function createAppointmentInDB(appointmentData: any) {
  ...
  const appointmentNumber = `OPD-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${newId.toString().padStart(3, '0')}`;
  return {
    id: newId,
...
async function updateAppointmentInDB(id: number, updateData: any) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!appointmentData.patient_id || !appointmentData.doctor_id || !appointmentData.appointment_date || !appointmentData.appointment_type) {
      return NextResponse.json(
