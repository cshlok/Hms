...
const CreateVisitSchema = z.object({
    ...
    visit_type: z.nativeEnum(OPDVisitType).default("WalkIn"),
    doctor_id: z.number().int().positive(),
    department: z.string().optional().nullable(),
    status: z.nativeEnum(OPDVisitStatus).optional().default("Waiting"),
...
export async function POST(request: Request) {
    ...
    try {
        ...
        const checks = await DB.batch([
            ...
            // Optional: Check if appointment exists and matches patient/doctor if provided
            visitData.appointment_id ? DB.prepare("SELECT appointment_id FROM Appointments WHERE appointment_id = ? AND patient_id = ? AND doctor_id = ? AND status NOT IN ('Completed', 'Cancelled', 'NoShow')").bind(visitData.appointment_id, visitData.patient_id, visitData.doctor_id) : null
