  ...
  const getStatusBadgeVariant = (status: string): AllowedBadgeVariant => {
    switch (status.toLowerCase()) {
      ...
      case "void":
        return "destructive";
      default:
        return "secondary"; // Default to secondary for unknown statuses
    }
  };
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">Total (₹)</TableHead> {/* Shortened label */}
              <TableHead className="text-right">Due (₹)</TableHead> {/* Shortened label */}
            ...
            {isLoading ? (
              ...
              Array.from({ length: 5 }).map((_, index) => (
                <TableRow key={`skeleton-${index}`}>
                  ...
                  <TableCell className="text-right"><Skeleton className="h-4 w-16 ml-auto" /></TableCell>
                  <TableCell><Skeleton className="h-6 w-20 rounded-full" /></TableCell> {/* Rounded skeleton for badge */}
              ...
              invoices.map((invoice) => (
                <TableRow key={invoice.id}>
                  ...
                  <TableCell className="text-right font-medium">{invoice.amount_due > 0 ? invoice.amount_due.toFixed(2) : "-"}</TableCell> {/* Show dash if 0 due */}
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    ...
                    {/* <Link href={`/dashboard/billing/invoices/${invoice.id}`} passHref> */}
                      <Button variant="ghost" size="icon" title="View Invoice Details"> {/* Added title */}
              ...
              <TableRow>
                <TableCell colSpan={7} className="h-24 text-center text-muted-foreground">
                  {searchTerm ? `No invoices found matching "${searchTerm}".` : "No invoices available."}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
