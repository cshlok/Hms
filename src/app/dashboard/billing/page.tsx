...
export default function BillingDashboardPage() {
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    try {
      ...
      setTotalCount(data?.totalCount ?? (Array.isArray(data?.invoices) ? data.invoices.length : 0)); // Use length as fallback if totalCount missing
    } catch (err) {
  ...
  const getStatusBadgeVariant = (status: string): AllowedBadgeVariant => {
    switch (status.toLowerCase()) {
      ...
      case "void":
        return "destructive";
      case "finalized": // Assuming finalized is a valid status
        return "outline"; // Map "finalized" to "outline"
      default:
  ...
  return (
    <div className="container mx-auto py-8 px-4 md:px-6 lg:px-8 space-y-6"> {/* Added lg:px-8 */}
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4"> {/* Responsive layout */}
        ...
        <div className="flex gap-2 w-full sm:w-auto"> {/* Button group */}
          <Link href="/dashboard/billing/service-items" passHref>
            <Button variant="outline" className="flex-1 sm:flex-none">Manage Services</Button>
          </Link>
          <Link href="/dashboard/billing/create-invoice" passHref>
            <Button className="flex-1 sm:flex-none"> {/* Full width on small screens */}
      ...
      <Card>
        <CardHeader className="flex flex-row justify-between items-center"> {/* Flex layout for header */}
          <div>
            <CardTitle>Recent Invoices</CardTitle>
            <CardDescription>Showing the latest {invoices.length} invoices.</CardDescription> {/* Dynamic description */}
          </div>
        ...
        <CardContent>
          ...
          <div className="rounded-md border overflow-x-auto"> {/* Added overflow-x-auto */}
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="min-w-[120px]">Invoice #</TableHead> {/* Added min-width */}
                  <TableHead className="min-w-[150px]">Patient</TableHead> {/* Added min-width */}
                  <TableHead>Date</TableHead>
                  <TableHead className="text-right">Total (₹)</TableHead>
                  <TableHead className="text-right">Due (₹)</TableHead>
                  <TableHead>Status</TableHead>
                  <TableHead className="text-right">Actions</TableHead> {/* Right align actions */}
                </TableRow>
              </TableHeader>
              <TableBody>
                {loading ? (
                  ...
                  Array.from({ length: 5 }).map((_, index) => (
                    <TableRow key={`skeleton-${index}`}>
                      ...
                      <TableCell className="text-right"><Skeleton className="h-8 w-16 ml-auto rounded" /></TableCell> {/* Rounded skeleton for button */}
                    </TableRow>
                  ))
                ) : invoices.length > 0 ? (
                  // Invoice Data Rows
                  invoices.map((invoice) => (
                    <TableRow key={invoice.id}>
                      ...
                      <TableCell className="font-medium">{invoice.patient_name || `ID: ${invoice.patient_id}`}</TableCell>
                      ...
                      <TableCell className="text-right font-medium">{invoice.amount_due > 0 ? invoice.amount_due.toFixed(2) : "-"}</TableCell>
                      <TableCell>
                        <Badge variant={getStatusBadgeVariant(invoice.status)} className="capitalize">
                          {invoice.status.replace("_", " ")}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-right"> {/* Right align actions */}
          ...
          {totalCount > invoices.length && !loading && (
            <div className="text-sm text-muted-foreground mt-4">
              Showing {invoices.length} of {totalCount} total invoices. <Link href="/dashboard/billing/invoices" className="underline">View all</Link>.
            </div>
          )}
        </CardContent>
      </Card>
