...
const getEsiBadgeVariant = (esi: number): "destructive" | "secondary" | "default" | "outline" => {
  switch (esi) {
    case 1: return "destructive"; // Highest urgency
    case 2: return "secondary";   // High urgency (mapped from warning)
    case 3: return "default";     // Moderate urgency (mapped from success)
    case 4: return "outline";     // Low urgency (mapped from info)
    case 5: return "outline";     // Lowest urgency (mapped from secondary)
...
export default function ERPatientTrackingBoard() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const mockData: ERPatient[] = [
          { id: "visit_1", patient_id: "p1", patient_name: "John Doe", mrn: "MRN001", age: 45, sex: "M", chief_complaint: "Chest Pain", arrival_time: new Date(Date.now() - 125 * 60 * 1000).toISOString(), location: "Room 3", esi: 2, assigned_physician: "Dr. Smith", assigned_nurse: "Nurse Joy", status: "Treatment", indicators: { lab_pending: true, rad_pending: true, critical_alert: "STEMI" } },
          { id: "visit_2", patient_id: "p2", patient_name: "Jane Smith", mrn: "MRN002", age: 68, sex: "F", chief_complaint: "Shortness of Breath", arrival_time: new Date(Date.now() - 30 * 60 * 1000).toISOString(), location: "Room 5", esi: 3, assigned_physician: "Dr. Jones", assigned_nurse: "Nurse Kim", status: "Assessment", indicators: { lab_ready: true, fall_risk: true } },
          { id: "visit_3", patient_id: "p3", patient_name: "Peter Pan", mrn: "MRN003", age: 32, sex: "M", chief_complaint: "Abdominal Pain", arrival_time: new Date(Date.now() - 65 * 60 * 1000).toISOString(), location: "Hallway Bed 1", esi: 4, assigned_physician: null, assigned_nurse: "Nurse Lee", status: "Awaiting Disposition", indicators: { rad_ready: true, consult_pending: true } },
          { id: "visit_4", patient_id: "p4", patient_name: "Alice Wonderland", mrn: "MRN004", age: 75, sex: "F", chief_complaint: "Weakness", arrival_time: new Date(Date.now() - 15 * 60 * 1000).toISOString(), location: "Triage Room 2", esi: 2, assigned_physician: null, assigned_nurse: null, status: "Triage", indicators: { critical_alert: "Stroke" } },
          { id: "visit_5", patient_id: "p5", patient_name: "Bob Builder", mrn: "MRN005", age: 50, sex: "M", chief_complaint: "Laceration", arrival_time: new Date(Date.now() - 200 * 60 * 1000).toISOString(), location: "Room 1", esi: 5, assigned_physician: "Dr. Smith", assigned_nurse: "Nurse Joy", status: "Discharged", indicators: {} }, // Example discharged patient (might be filtered out by API)
        ];
  ...
  const filteredPatients = patients.filter(p => {
    const nameMatch = filterName === "" || p.patient_name.toLowerCase().includes(filterName.toLowerCase()) || p.mrn.includes(filterName);
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b dark:border-gray-700">
                ...
                <TableHead className="text-gray-700 dark:text-gray-300">ESI</TableHead>
                {/* <TableHead>Wait</TableHead> */} {/* Wait time might be complex, removing for now */}
                <TableHead className="text-gray-700 dark:text-gray-300">LOS</TableHead>
                <TableHead className="text-gray-700 dark:text-gray-300">Staff</TableHead>
                <TableHead className="text-gray-700 dark:text-gray-300">Status</TableHead>
                <TableHead className="text-gray-700 dark:text-gray-300">Indicators</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoading ? (
                ...
                Array.from({ length: 5 }).map((_, index) => (
                  <TableRow key={`skeleton-${index}`} className="border-b dark:border-gray-700 last:border-b-0">
                ...
                filteredPatients.map((patient) => (
                  <TableRow key={patient.id} className="border-b dark:border-gray-700 last:border-b-0 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <TableCell>
                      <div className="font-medium text-gray-900 dark:text-gray-100">{patient.patient_name}</div>
                      <div className="text-sm text-muted-foreground dark:text-gray-400">{patient.mrn} ({patient.age}{patient.sex})</div>
                    </TableCell>
                    <TableCell className="max-w-[150px] truncate text-gray-700 dark:text-gray-300">{patient.chief_complaint}</TableCell>
                    <TableCell className="text-gray-700 dark:text-gray-300">{patient.location}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-gray-700 dark:text-gray-300">{calculateTimeDiff(patient.arrival_time)}</TableCell> 
                    <TableCell>
                      <div className="text-sm text-gray-700 dark:text-gray-300">MD: {patient.assigned_physician || "N/A"}</div>
                      <div className="text-sm text-gray-700 dark:text-gray-300">RN: {patient.assigned_nurse || "N/A"}</div>
                    </TableCell>
                    <TableCell className="text-gray-700 dark:text-gray-300">{patient.status}</TableCell>
                    <TableCell>
                      <div className="flex space-x-1.5">
                        ...
                        {patient.indicators.critical_alert && (
                          <Tooltip>
                            ...
                            <TooltipContent>Alert: {patient.indicators.critical_alert}</TooltipContent>
                          </Tooltip>
                        )}
