...
interface ERRadiologyOrderModalProps {
  isOpen: boolean;
  onClose: () => void;
  visitData?: {
    id: string;
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Select procedure for patient: {visitData?.patientName || "N/A"} (Visit: {visitData?.id || "N/A"})
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Clinical Notes / Reason for Exam (Optional)</FormLabel>
                  <FormControl>
                    <Input placeholder="e.g., R/O Pneumonia, Trauma assessment" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
