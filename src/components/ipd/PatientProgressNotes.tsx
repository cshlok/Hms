    ...
    const fetchData = async (): Promise<void> => {
      ...
      try {
        ...
        const mockProgressNotes: ProgressNote[] = [
          {
            id: "pn_001", admission_id: admissionId, note_date: new Date(Date.now() - 3600000 * 6).toISOString(), // 6 hours ago
            subjective: "Patient reports feeling slightly better. Cough less frequent.",
            objective: "Temp 37.2C, HR 85, RR 18, SpO2 98% on RA. Lungs clearer on auscultation.",
            assessment: "Improving pneumonia.",
            plan: "Continue current antibiotics. Monitor respiratory status. Encourage ambulation.",
            doctor_id: "doc_101", doctor_first_name: "Alice", doctor_last_name: "Smith", created_at: new Date().toISOString()
          },
          {
            id: "pn_002", admission_id: admissionId, note_date: new Date(Date.now() - 86400000).toISOString(), // 1 day ago
            subjective: "Patient complaining of persistent cough and mild shortness of breath.",
            objective: "Temp 37.8C, HR 90, RR 22, SpO2 95% on 2L NC. Crackles heard in right lower lobe.",
            assessment: "Community-acquired pneumonia.",
            plan: "Continue IV Ceftriaxone. Monitor O2 saturation. Chest physiotherapy.",
            doctor_id: "doc_102", doctor_first_name: "Bob", doctor_last_name: "Johnson", created_at: new Date(Date.now() - 86400000).toISOString()
          },
        ];
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      if (!formData.subjective && !formData.objective && !formData.assessment && !formData.plan) {
        throw new Error("At least one section (Subjective, Objective, Assessment, Plan) must be filled.");
      }
      ...
      <Card>
        ...
        <CardContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="objective" className="font-medium">Objective (Findings)</Label>
              <Textarea
                id="objective"
                name="objective"
            ...
            <div className="space-y-2">
              <Label htmlFor="assessment" className="font-medium">Assessment (Diagnosis/Impression)</Label>
              <Textarea
                id="assessment"
                name="assessment"
            ...
            <div className="space-y-2">
              <Label htmlFor="plan" className="font-medium">Plan (Treatment/Management)</Label>
              <Textarea
                id="plan"
                name="plan"
                value={formData.plan}
