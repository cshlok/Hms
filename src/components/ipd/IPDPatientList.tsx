...
const IPDPatientList: React.FC<IPDPatientListProps> = ({ onViewPatient }) => {
  ...
  useEffect(() => {
    const fetchPatients = async () => {
      try {
        ...
        const validData = Array.isArray(data) ? data.filter(p => p.id && p.patient_id) : [];
        ...
        setError(`Failed to load inpatient list: ${message}. Please try again later.`);
  ...
  const handleViewClick = (admissionIdStr: string, patientIdStr: string) => {
    ...
    if (!isNaN(admissionId) && !isNaN(patientId)) {
      onViewPatient(admissionId, patientId);
    } else {
      console.error("Invalid ID format for patient or admission:", admissionIdStr, patientIdStr);
                ...
                <TableRow key={patient.id}>
                  ...
                  <TableCell>
                    ...
                    <Button 
                      size="sm" 
                      variant="outline"
                      onClick={() => handleViewClick(patient.id, patient.patient_id)}
                    >
                      View Details
                    </Button>
                  </TableCell>
