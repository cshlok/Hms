...
const NursingNotes: React.FC<NursingNotesProps> = ({ admissionId }) => {
  const [nursingNotes, setNursingNotes] = useState<NursingNote[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [formData, setFormData] = useState<FormData>({
    vital_signs: JSON.stringify(defaultVitalSigns, null, 2),
    intake_output: JSON.stringify(defaultIntakeOutput, null, 2),
    medication_given: "",
    procedures: "",
    notes: "",
  });
    ...
    const fetchNursingNotes = async (): Promise<void> => {
      ...
      try {
        ...
        const mockNotes: NursingNote[] = [
          {
            id: "nn_001",
            note_date: new Date(Date.now() - 7200000).toISOString(), // 2 hours ago
            nurse_first_name: "Bob",
            nurse_last_name: "Johnson",
            vital_signs: JSON.stringify({ temperature: "37.1 C", pulse: "78 bpm", blood_pressure: "122/78 mmHg", oxygen_saturation: "98%" }),
            intake_output: JSON.stringify({ oral_intake: "500ml water", iv_fluids: "100ml NS", urine_output: "300ml" }),
        ...
        const mockPatientInfo: AdmissionInfo = {
          ...
          admission_date: new Date(Date.now() - 86400000).toISOString(), // Yesterday
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label htmlFor="vital_signs" className="font-medium text-sm">Vital Signs (JSON)</label>
                <Textarea
                  id="vital_signs"
                  name="vital_signs"
              ...
              <div className="space-y-2">
                <label htmlFor="intake_output" className="font-medium text-sm">Intake/Output (JSON)</label>
                <Textarea
                  id="intake_output"
                  name="intake_output"
            ...
            <div className="space-y-2">
              <label htmlFor="medication_given" className="font-medium text-sm">Medications Given</label>
              <Textarea
                id="medication_given"
            ...
            <div className="space-y-2">
              <label htmlFor="procedures" className="font-medium text-sm">Procedures Performed</label>
              <Textarea
                id="procedures"
                name="procedures"
            ...
            <div className="space-y-2">
              <label htmlFor="notes" className="font-medium text-sm">Notes <span className="text-red-500">*</span></label>
              <Textarea
                id="notes"
                name="notes"
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-gray-500 p-4 text-center">No nursing notes found for this admission.</div>
          ) : (
            <div className="space-y-6">
              {nursingNotes.map((note) => {
                ...
                return (
                  <div key={note.id} className="border rounded-md p-4 shadow-sm bg-white">
