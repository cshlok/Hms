...
interface IPDPharmacyIntegrationProps {
...
const IPDPharmacyIntegration: React.FC<IPDPharmacyIntegrationProps> = ({ admissionId, patientId }) => {
    ...
    const fetchData = async (): Promise<void> => {
      ...
      try {
        ...
        const mockRecords: AdministrationRecord[] = [
          { id: "admin_001", schedule_id: "sched_001", prescription_item_id: "item_003", medication_name: "Amoxicillin 250mg", administered_at: new Date(Date.now() - 7200000).toISOString(), administered_by: "Nurse Jane", notes: "Patient took medication without issues." },
        ];
        ...
        console.error("Error fetching IPD pharmacy data:", err);
  ...
  const handleAdministerMedication = async (scheduleItem: MedicationScheduleItem): Promise<void> => {
    if (!admissionId) {
    ...
    try {
      ...
      setAdministrationRecords((prevRecords) => [newRecord, ...prevRecords]);
  ...
  const getDosageForScheduleItem = (prescriptionItemId: string): string => {
    for (const prescription of prescriptions) {
      const item = prescription.items.find(pItem => pItem.id === prescriptionItemId);
      if (item) {
        return item.dosage;
      }
    }
  ...
  if (loading) {
    return <div className="flex justify-center items-center h-64"><Loader2 className="h-8 w-8 animate-spin text-primary" /> Loading medication schedule...</div>;
  }

  if (error) {
    return <div className="text-center text-red-500 p-4" role="alert">{error}</div>;
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden mt-6">
      ...
      <div className="p-6 space-y-8">
        {/* Medication Schedule */}
        <div>
          <h3 className="text-md font-medium text-gray-700 mb-3">Scheduled Medications</h3>
                  ...
                  {medicationSchedule.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td className="px-4 py-2 whitespace-nowrap">
                        <span
                          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${item.status === "administered"
                              ? "bg-green-100 text-green-800"
                              : item.status === "skipped" || item.status === "held"
        ...
        <div>
          <h3 className="text-md font-medium text-gray-700 mb-3">Administration History</h3>
