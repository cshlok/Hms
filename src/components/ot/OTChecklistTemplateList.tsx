...
export default function OTChecklistTemplateList() {
        ...
        const mockData: ChecklistTemplate[] = [
          {
            ...
            phase: "pre-op",
            items: [
              { id: "item-1", text: "Patient confirmed identity, site, procedure, consent?" },
              { id: "item-2", text: "Site marked?" },
              { id: "item-3", text: "Anesthesia safety check completed?" },
              { id: "item-4", text: "Pulse oximeter on patient and functioning?" },
          ...
          },
          {
            ...
            phase: "intra-op",
            items: [
              { id: "item-1", text: "Confirm all team members introduced by name and role?" },
              { id: "item-2", text: "Confirm patient name, procedure, incision site?" },
              { id: "item-3", text: "Antibiotic prophylaxis given within last 60 minutes?" },
          ...
          },
          {
            ...
            phase: "post-op",
            items: [
              { id: "item-1", text: "Nurse confirms name of procedure recorded?" },
              { id: "item-2", text: "Instrument, sponge, needle counts correct?" },
              { id: "item-3", text: "Specimen labelling correct?" },
              { id: "item-4", text: "Equipment problems identified?" },
              { id: "item-5", text: "Key concerns for recovery and management reviewed?" },
            ],
  ...
  const getPhaseBadge = (phase: string) => {
    switch (phase.toLowerCase()) {
      case "pre-op": return <Badge className="bg-blue-100 text-blue-800">Pre-Op</Badge>;
      case "intra-op": return <Badge className="bg-yellow-100 text-yellow-800">Intra-Op</Badge>;
      case "post-op": return <Badge className="bg-green-100 text-green-800">Post-Op</Badge>;
        ...
        {!loading && !error && (
          <Table>
            ...
            <TableBody>
              {templates.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={4} className="text-center">No checklist templates found.</TableCell>
                </TableRow>
              ) : (
                  ...
                  <TableRow key={template.id}>
                    ...
                    <TableCell>
                      <div className="flex space-x-2">
                        <Button variant="outline" size="icon" title="View/Edit Template">
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button variant="destructive" size="icon" title="Delete Template">
