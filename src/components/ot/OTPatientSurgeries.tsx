...
export default function OTPatientSurgeries({ patientId }: OTPatientSurgeriesProps) {
  ...
  useEffect(() => {
    const fetchPatientSurgeries = async () => {
      try {
        ...
        setSurgeries(mockData.filter(s => s.status === "scheduled" || s.status === "confirmed"));

      } catch (err: unknown) { // FIX: Use unknown for error type
        const message = err instanceof Error ? err.message : "An unknown error occurred";
        console.error("Error fetching patient surgeries:", message);
        setError(message);
        toast({ title: "Error", description: `Failed to load surgeries: ${message}`, variant: "destructive" });
      } finally {
        setLoading(false);
      }
    };
  ...
  const getStatusBadge = (status: Surgery["status"]) => {
    switch (status) {
      ...
      default: return <Badge>{status}</Badge>; // Fallback for unexpected statuses
    }
  };
              ...
              <TableBody>
                ...
                {surgeries.map((surgery: Surgery) => (
                  <TableRow key={surgery.id}>
                    ...
                    <TableCell className="font-medium">{surgery.surgery_name}</TableCell>
                    ...
                    <TableCell className="text-right">
                      <Link href={`/dashboard/ot/bookings/${surgery.id}`} passHref>
