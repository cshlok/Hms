...
interface OTStaffAssignmentProps {
  bookingId: string;
}
...
export default function OTStaffAssignment({ bookingId }: OTStaffAssignmentProps) {
  ...
  const fetchAssignedStaff = useCallback(async () => {
    try {
      ...
      const mockAssigned: AssignedStaff[] = [
        { assignment_id: "assign-1", user_id: "user-1", user_name: "Dr. Alice Brown", role: "Lead Surgeon" },
        { assignment_id: "assign-2", user_id: "user-3", user_name: "Dr. Charlie Green", role: "Anesthesiologist" },
        { assignment_id: "assign-3", user_id: "user-5", user_name: "Nurse Eve Adams", role: "Scrub Nurse" },
      ];
  ...
  const handleAddStaff = async () => {
    if (!selectedUser || !selectedRole) {
      toast({ title: "Error", description: "Please select a user and assign a role.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      setAssignedStaff(prev => [...prev, newAssignment]);
      setSelectedUser("");
      setSelectedRole("");
      ...
      <CardContent>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6 p-4 border rounded-md bg-muted/40">
          ...
          <div className="flex items-end">
            <Button onClick={handleAddStaff} disabled={isAdding || !selectedUser || !selectedRole} className="w-full md:w-auto">
              {isAdding ? "Adding..." : <><PlusCircle className="mr-2 h-4 w-4" /> Add Staff</>}
            </Button>
          </div>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {assignedStaff.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={3} className="text-center">No staff assigned yet.</TableCell>
                </TableRow>
              ) : (
