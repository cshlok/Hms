...
export default function OTBookingList() {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        ...
        setBookings(mockData.filter(b => 
            (!filters.status || b.status === filters.status) &&
            (!filters.date || format(new Date(b.scheduled_start_time), 'yyyy-MM-dd') === filters.date)
  ...
  const getStatusBadge = (status: string) => {
    switch (status.toLowerCase()) {
      ...
      case "postponed": return <Badge className="bg-purple-100 text-purple-800">Postponed</Badge>;
              ...
              {bookings.length === 0 ? (
                ...
                bookings.map((booking) => (
                  <TableRow key={booking.id}>
                    <TableCell>
                        {format(new Date(booking.scheduled_start_time), 'HH:mm')} - {format(new Date(booking.scheduled_end_time), 'HH:mm')}
                    </TableCell>
                    <TableCell>{booking.patient_name} ({booking.patient_mrn})</TableCell>
                    ...
                    <TableCell>
                      <div className="flex space-x-2">
                        ...
                        {booking.status !== 'completed' && booking.status !== 'cancelled' && (
                            <Button variant="destructive" size="icon" title="Cancel Booking">
