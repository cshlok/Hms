...
- [x] **Phase 1: Configuration & High-Impact Module Errors**
    ...
    - [ ] Fix `TS2307: Cannot find module` errors (e.g., `env`, `nanoid`). Check paths and install missing types (`@types/nanoid` if needed).
    - [ ] Fix `TS2305: Module has no exported member` errors (e.g., `Session`, `SessionUser` in `@/lib/session`).
...
- [ ] **Phase 2: Common Type Mismatches & Unknowns**
    - [ ] Fix `TS18046: '...' is of type 'unknown'` errors (e.g., `insertResult.meta`). Provide explicit types for DB results/metadata.
