...
const testIPDBeds = async () => {
  ...
  try {
    ...
    if (data.beds && data.beds.length > 0) {
      ...
      const requiredFields = ['id', 'bed_number', 'room_number', 'ward', 'status'];
      const missingFields = requiredFields.filter(field => !(field in sampleBed));
      
      if (missingFields.length > 0) {
        console.error(`Missing required fields in bed data: ${missingFields.join(', ')}`);
      } else {
...
const testIPDAdmissions = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch admissions: ${response.status} ${response.statusText}`);
    }
    ...
    if (data.admissions && data.admissions.length > 0) {
      ...
      // Check required fields
      const requiredFields = ['id', 'admission_number', 'patient_id', 'bed_id', 'admission_date', 'status'];
      const missingFields = requiredFields.filter(field => !(field in sampleAdmission));
      
      if (missingFields.length > 0) {
        console.error(`Missing required fields in admission data: ${missingFields.join(', ')}`);
      } else {
    ...
    console.log('Note: POST test is simulated and not actually executed to avoid creating test data');
...
const testIPDProgressNotes = async () => {
  ...
  try {
    ...
    console.log('Note: This is a simulation and not actually executed to avoid using invalid IDs');
...
const testDashboardIPDStats = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch IPD stats: ${response.status} ${response.statusText}`);
    }
    ...
    // Check required fields
    const requiredFields = ['activeAdmissions', 'availableBeds', 'occupancyRate'];
    const missingFields = requiredFields.filter(field => !(field in data));
    
    if (missingFields.length > 0) {
      console.error(`Missing required fields in IPD stats: ${missingFields.join(', ')}`);
    } else {
